"""Bollinger Band mean-reversion backtest -- an alternative to the RSI signal.

Same underlying idea as the RSI strategy (buy when statistically oversold,
sell/short when statistically overbought, exit on reversion to the mean),
but measured directly in price/volatility terms instead of RSI:

    middle = N-bar SMA
    upper  = middle + K * rolling_std
    lower  = middle - K * rolling_std

    Long entry:  close < lower.  Long exit:  close >= middle, or stop hit.
    Short entry (opt-in): close > upper.  Short exit: close <= middle, or stop hit.

This is a different math family (price distance in standard deviations,
rather than RSI's momentum oscillator), worth comparing directly against
...
def simulate_bollinger(
    ...
    middle_series = df["Close"].rolling(bb_period).mean()
    std_series = df["Close"].rolling(bb_period).std()
    middle = middle_series.to_numpy(dtype=np.float64)
    upper = (middle_series + bb_std_mult * std_series).to_numpy(dtype=np.float64)
    lower = (middle_series - bb_std_mult * std_series).to_numpy(dtype=np.float64)
    ...
    for i in range(n):
        ...
        if direction is not None:
            ...
            if direction == "long":
                ...
                elif bands_ready and close >= mid:
        ...
        if direction is None and bands_ready:
            new_direction = None
            if close < lower[i]:
                new_direction = "long"
            elif enable_short and close > upper[i]:
                new_direction = "short"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest a Bollinger Band mean-reversion strategy.")
    ...
    parser.add_argument("--bb-period", type=int, default=20, help="SMA/std period in bars.")
    parser.add_argument("--bb-std-mult", type=float, default=2.0, help="Band width as a multiple of rolling std.")
    parser.add_argument("--stop-loss-pct", type=float, default=5.0)
    parser.add_argument("--position-size-r", type=float, default=0.5)
    parser.add_argument(
        "--enable-short",
        action="store_true",
        help="Backtest-only: also take short entries above the upper band. Kraken spot has no "
...
def main() -> None:
    ...
    print(f"Bars:                {len(df)}")
    print(f"BB period / std mult: {args.bb_period} / {args.bb_std_mult}")
